...
public class InfoHudViewHolder {
    ...
    private static String formatedDurationMilli(long duration) {
        if (duration >=  1000) {
            return String.format(Locale.US, "%.2f msec", ((float)duration) / 1000);
        } else {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case MSG_UPDATE_HUD: {
                    ...
                    // JS_MODIFY
                    setRowValue(R.string.js_br, String.format(Locale.US, "%s, %s", mp.getCurrentBitrateName(), mp.getAdaptive() == 0 ? "fixed" : "adaptive"));
                    setRowValue(R.string.js_ve, mp.getCurrentViewName());
                    setRowValue(R.string.js_ao, mp.getCurrentAudioName());
                    // JS_MODIFY END
