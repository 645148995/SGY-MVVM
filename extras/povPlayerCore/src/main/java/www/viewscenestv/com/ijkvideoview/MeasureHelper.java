...
public final class MeasureHelper {
    ...
    public void setDisplayArea(Rect source) {
        ...
        ViewCompat.setClipBounds(view, sourceAreaToScreen(source));
        view.requestLayout();
    }

    @Nullable
    public Rect sourceAreaToScreen(Rect video) {
        if (video == null)
            return null;

        float vscale = (float) mMeasuredHeight / (float) mVideoHeight;
        float hscale = (float) mMeasuredWidth / (float) mVideoWidth;

        Rect screen = new Rect(video);
        screen.top = (int) (screen.top * vscale);
        screen.bottom = (int) (screen.bottom * vscale);
        screen.left = (int) (screen.left * hscale);
        screen.right = (int) (screen.right * hscale);
        return screen;
    }
            ...
            if (widthSpecMode == View.MeasureSpec.AT_MOST && heightSpecMode == View.MeasureSpec.AT_MOST) {
                ...
                switch (mCurrentAspectRatio) {
                    ...
                    case IRenderView.AR_ASPECT_FILL_PARENT:
                        ...
                        break;
                    case IRenderView.AR_FILL_WIDTH_ON_TOP:
                        width = widthSpecSize;
                        height = (int) (width / displayAspectRatio);
                        break;
                    case IRenderView.AR_ASPECT_WRAP_CONTENT:
    ...
    public static String getAspectRatioText(Context context, int aspectRatio) {
        ...
        switch (aspectRatio) {
            ...
            case IRenderView.AR_4_3_FIT_PARENT:
                ...
                break;
            case IRenderView.AR_FILL_WIDTH_ON_TOP:
                text = context.getString(R.string.VideoView_ar_fill_parent_width_on_top);
                break;
            default:
